...
- **Next.js**: 16.2.3 — before writing any code, read `node_modules/next/dist/docs/` for current API. APIs differ from earlier versions.
...
- Canonical URLs and `hreflang` for all `[lang]` routes (ce / ru / en) are mandatory.
...
- Routing is handled via `[lang]` dynamic segment (ce / ru / en).
- Translations come from `src/dictionaries/`. Access via `src/i18n/` utilities.
